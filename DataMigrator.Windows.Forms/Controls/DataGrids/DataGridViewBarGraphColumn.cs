...
namespace DataMigrator.Windows.Forms.Controls.DataGrids
{
    public class DataGridViewBarGraphCell : DataGridViewTextBoxCell
    {
          ...
          DataGridViewPaintParts paintParts)
        {
            ...
            RectangleF newRect = new RectangleF(cellBounds.X + HORIZONTALOFFSET, cellBounds.Y + VERTOFFSET,
              Convert.ToSingle(cellValue), cellBounds.Height - (VERTOFFSET * 2));
            ...
            PointF textStart = new PointF(Convert.ToSingle(HORIZONTALOFFSET + cellValue + SPACER),
        ...
        public void CalcMaxValue()
        {
            if (needsRecalc)
            {
                ...
                for (int rowIndex = 0; rowIndex < this.DataGridView.Rows.Count; rowIndex++)
                {
                    ...
                    try
                    { MaxValue = System.Math.Max(MaxValue, Convert.ToInt64(row.Cells[colIndex].Value)); }
                    catch
                    { MaxValue = 1; }
                }
                needsRecalc = false;
