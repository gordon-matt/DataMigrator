...
{
    private static readonly List<(FieldType FieldType, DbType DbType)> fieldTypes = new();
    ...
    public FieldType GetDataMigratorFieldType(DbType providerFieldType) => dbTypes.First(x => x.DbType == providerFieldType).FieldType;

    public DbType GetDataProviderFieldType(FieldType fieldType) => fieldTypes.First(x => x.FieldType == fieldType).DbType;
